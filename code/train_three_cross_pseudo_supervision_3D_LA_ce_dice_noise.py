...
from dataloaders.la_heart import (LAHeart, CenterCrop, RandomCrop,
                                   RandomRotFlip, ToTensor,
                                   TwoStreamBatchSampler)
from networks.net_factory_3d import net_factory_3d
from utils_ import metrics
from utils1 import losses, metrics, ramps
from test_LA import val_all_case

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str,
                    default='/mnt/ai2021/zxg/dataset/2018LA_Seg_Training', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='Cross_Pseudo_Supervision_sup_dice_dice_LA', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            noise1 = torch.clamp(torch.randn_like(volume_batch) * 0.1, -0.1, 0.1)
            volume_batch1 = volume_batch + noise1

            noise2 = torch.clamp(torch.randn_like(
                volume_batch) * 0.1, -0.2, 0.2)
            volume_batch2 = volume_batch + noise2

            noise3 = torch.clamp(torch.randn_like(
                volume_batch) * 0.1, -0.3, 0.3)
            volume_batch3 = volume_batch + noise3

            outputs1 = model1(volume_batch1)
            outputs_soft1 = torch.softmax(outputs1, dim=1)
            outputs2 = model2(volume_batch2)
            outputs_soft2 = torch.softmax(outputs2, dim=1)
            outputs3 = model3(volume_batch3)
            outputs_soft3 = torch.softmax(outputs3, dim=1)
            consistency_weight = get_current_consistency_weight(iter_num // 150)

            loss1 = 0.5 * (ce_loss(outputs1[:args.labeled_bs], label_batch[:args.labeled_bs, 1, :, :, :])
                           + losses.dice_loss(outputs_soft1[:args.labeled_bs], label_batch[:args.labeled_bs]))
            loss2 = 0.5 * (losses.dice_loss(outputs2[:args.labeled_bs],
                                     label_batch[:args.labeled_bs, 1, :, :, :]) + losses.dice_loss(
                outputs_soft2[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))

            loss3 = 0.5 * (losses.dice_loss(outputs3[:args.labeled_bs],
                                     label_batch[:args.labeled_bs, 1, :, :, :]) + losses.dice_loss(
            ...
            pseudo_supervision1 = losses.dice_loss(outputs1[args.labeled_bs:], pseudo_outputs2)
            pseudo_supervision2 = losses.dice_loss(outputs2[args.labeled_bs:], pseudo_outputs3)
            pseudo_supervision3 = losses.dice_loss(outputs3[args.labeled_bs:], pseudo_outputs1)
            ...
            if iter_num % 50 == 0:
                ...
                image = label_batch[0, :, :, 20:61:10].permute(3, 0, 1, 2)
                grid_image = make_grid(image, 5, normalize=False)
            ...
            if iter_num > 0 and iter_num % 2 == 0:
                model1.eval()
                avg_metric1 = val_all_case(
                    model1, args.root_path, test_list="val_list.txt", num_classes=2, patch_size=args.patch_size,
                ...
                model1.train()
                model2.eval()
                avg_metric2 = val_all_case(
                    model2, args.root_path, test_list="val_list.txt", num_classes=2, patch_size=args.patch_size,
                    stride_xy=18, stride_z=4)
                ...
                model3.eval()
                avg_metric3 = val_all_case(
                    model3, args.root_path, test_list="val_list.txt", num_classes=2, patch_size=args.patch_size,
                    stride_xy=18, stride_z=4)
...
def test():
    # snapshot_path = '/mnt/ai2021/zxg/semi-supervised/SSL4MIS/model/LA/LA_8_Cross_Pseudo_Supervision_sup_ce_dice_8_8/'
    num_classes = 2
    ...
    save_mode_path = os.path.join(
            snapshot_path, 'unet_3D_best_model1.pth')
