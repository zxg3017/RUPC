...
def test_all_case_sdf(net, base_dir, method="unet_3D", test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, test_save_path=None):
...
def test_all_case(net, base_dir, method="unet_3D", test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24, test_save_path=None):
    ...
    print("Testing begin")
    metrics_for_csv = []
    metrics_for_csv.append(['case_id', 'dice', 'jc', 'ravd', 'hd', 'asd'])
    with open(test_save_path + "/{}.txt".format(method), "a") as f:
        for image_path in tqdm(image_list):
            ids = image_path.split("/")[-1].replace(".h5", "")
            h5f = h5py.File(image_path, 'r')
            ...
            total_metric[0, :] += metric
            f.writelines("{},{},{},{},{},{}\n".format(
                ids, metric[0], metric[1], metric[2], metric[3], metric[4]))
            ...
            sitk.WriteImage(lab_itk, test_save_path +
                            "/{}_lab.nii.gz".format(ids))
        f.writelines("Mean metrics,{},{},{},{},{}".format(total_metric[0, 0] / len(image_list), total_metric[0, 1] / len(
            image_list), total_metric[0, 2] / len(image_list), total_metric[0, 3] / len(image_list), total_metric[0, 4] / len(image_list)))
    f.close()
    print("Testing end")
    return total_metric / len(image_list)
...
def calculate_metric_percase(pred, gt):
    ...
    return np.array([dice, jc, ravd, hd, asd])
