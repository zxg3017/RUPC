...
def val_all_case_sdf(net, base_dir, test_list="full_test.list", num_classes=4, patch_size=(48, 160, 160), stride_xy=32, stride_z=24):
    with open(base_dir + '/{}'.format(test_list), 'r') as f:
        image_list = f.readlines()
    image_list = [base_dir + "/{}/mri_norm2.h5".format(
        item.replace('\n', '').split(",")[0]) for item in image_list]
    ...
    for image_path in tqdm(image_list):
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        prediction = val_single_case_sdf(
            net, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        for i in range(1, num_classes):
            total_metric[i-1, :] += val_cal_metric(label == i, prediction == i)
    print("Validation end")
...
def val_single_case_sdf(net, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
