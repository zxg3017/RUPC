...
parser.add_argument('--exp', type=str,
                    default='BraTs2019_Cross_Pseudo_Supervision_init3_sup_dice', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss1 = 0.5 * (dice_loss(outputs1[:args.labeled_bs],
                                     label_batch[:][:args.labeled_bs].unsqueeze(1)) + dice_loss(
                outputs_soft1[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))
            loss2 = 0.5 * (dice_loss(outputs2[:args.labeled_bs],
                                     label_batch[:][:args.labeled_bs].unsqueeze(1)) + dice_loss(
                outputs_soft2[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))

            loss3 = 0.5 * (dice_loss(outputs3[:args.labeled_bs],
                                     label_batch[:][:args.labeled_bs].unsqueeze(1)) + dice_loss(
                outputs_soft3[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1)))
