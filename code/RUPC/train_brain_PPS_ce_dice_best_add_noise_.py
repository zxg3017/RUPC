...
def gaussian_normal_init_weight(model):
    for m in model.modules():
        if isinstance(m, nn.Conv3d):
...
def train(args, snapshot_path):
    ...
    net3 = net_factory_3d(net_type=args.model, in_chns=1, class_num=num_classes).cuda()
    model1 = kaiming_normal_init_weight(net1)
    model2 = xavier_normal_init_weight(net2)
    model3 = gaussian_normal_init_weight(net3)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            unlabeled_volume_batch1 = unlabeled_volume_batch + noise1

            noise2 = torch.clamp(torch.randn_like(
                unlabeled_volume_batch) * 0.1, -0.2, 0.2)
            unlabeled_volume_batch2 = unlabeled_volume_batch + noise2

            noise3 = torch.clamp(torch.randn_like(
                unlabeled_volume_batch) * 0.1, -0.2, 0.2)
            unlabeled_volume_batch3 = unlabeled_volume_batch + noise3


            ## model 1 for labeled data
            outputs1_labeled = model1(labeled_volume_batch)
            outputs_soft1_labeled = torch.softmax(outputs1_labeled, dim=1)
            ## model 1 for unlabeled data
            outputs1_unlabeled = model1(unlabeled_volume_batch1)
            outputs_soft1_unlabeled = torch.softmax(outputs1_unlabeled, dim=1)

            ## model 2 for labeled data
            outputs2_labeled = model2(labeled_volume_batch)
            outputs_soft2_labeled = torch.softmax(outputs2_labeled, dim=1)
            ## model 2 for unlabeled data
            outputs2_unlabeled = model2(unlabeled_volume_batch2)
            outputs_soft2_unlabeled = torch.softmax(outputs2_labeled, dim=1)

            ## model 3 for labeled data
            outputs3_labeled = model3(labeled_volume_batch)
            outputs_soft3_labeled = torch.softmax(outputs3_labeled, dim=1)
            ## model 3 for unlabeled data
            outputs3_unlabeled = model3(unlabeled_volume_batch3)
            outputs_soft3_unlabeled = torch.softmax(outputs3_labeled, dim=1)

            consistency_weight = get_current_consistency_weight(iter_num // 150)

            loss1 = 0.5 * (ce_loss(outputs1_labeled,
                                   label_batch[:][:args.labeled_bs].long()) + dice_loss(
                outputs_soft1_labeled, label_batch[:args.labeled_bs].unsqueeze(1)))
            loss2 = 0.5 * (ce_loss(outputs2_labeled,
                                   label_batch[:][:args.labeled_bs].long()) + dice_loss(
                outputs_soft2_labeled, label_batch[:args.labeled_bs].unsqueeze(1)))

            loss3 = 0.5 * (ce_loss(outputs3_labeled,
                                   label_batch[:][:args.labeled_bs].long()) + dice_loss(
                outputs_soft3_labeled, label_batch[:args.labeled_bs].unsqueeze(1)))

            pseudo_outputs1 = torch.argmax(outputs_soft1_unlabeled.detach(), dim=1, keepdim=False)
            pseudo_outputs2 = torch.argmax(outputs_soft2_unlabeled.detach(), dim=1, keepdim=False)
            pseudo_outputs3 = torch.argmax(outputs_soft3_unlabeled.detach(), dim=1, keepdim=False)

            pseudo_supervision1 = dice_loss(outputs1_unlabeled, pseudo_outputs2.unsqueeze(1))
            pseudo_supervision2 = dice_loss(outputs2_unlabeled, pseudo_outputs3.unsqueeze(1))
            pseudo_supervision3 = dice_loss(outputs3_unlabeled, pseudo_outputs1.unsqueeze(1))

            model1_loss = loss1 + consistency_weight * pseudo_supervision1
            model2_loss = loss2 + consistency_weight * pseudo_supervision2
            model3_loss = loss3 + consistency_weight * pseudo_supervision3
            ...
            if iter_num % 50 == 0:
                ...
                writer.add_image('train/Model2_Predicted_label',
                                 grid_image, iter_num)
                image = outputs_soft3[0, 0:1, :, :, 20:61:10].permute(
                    3, 0, 1, 2).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image('train/Model3_Predicted_label',
                                 grid_image, iter_num)
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                avg_metric3 = test_all_case(
                    model3, args.root_path, test_list="val.txt", num_classes=2, patch_size=args.patch_size,
                    stride_xy=64, stride_z=64)
                if avg_metric3[:, 0].mean() > best_performance3:
                    best_performance3 = avg_metric3[:, 0].mean()
                    save_mode_path = os.path.join(snapshot_path,
                ...
                writer.add_scalar('info/model3_val_dice_score',
                              avg_metric3[0, 0], iter_num)
                writer.add_scalar('info/model3_val_hd95',
                              avg_metric3[0, 1], iter_num)
                logging.info(
                    'iteration %d : model3_dice_score : %f model3_hd95 : %f' % (
                        iter_num, avg_metric3[0, 0].mean(), avg_metric3[0, 1].mean()))
                model3.train()
